...
export class DailytrackerPage implements OnInit {
  ...
  ngOnInit() {
    this.userDetails = this.service.getUsers() //getting details about users smoking habit
  ...
  onChange() { // function that gets run when user enters the amount the smoked that day (submit button)
    ...
    var cigSmokedToday = parseFloat((costPerCig * this.numcigtoday).toFixed(2))
    this.moneySaved = parseFloat((oldHabitcost - cigSmokedToday).toFixed(2));
    this.moneySpent = parseFloat((costPerCig * this.numcigtoday).toFixed(2));
    ...
    //if else statements to alert different messages depending on the amount of cigarettes the user has smoked that day.
